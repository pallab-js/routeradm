...
def _write_wireguard_config(config: str) -> bool:
    try:
        ...
        with open(wg_conf, "w") as f:
            f.write(config)
        set_permissions(wg_conf, 0o600, "vpn")
...
def start_wireguard() -> bool:
    result = run(["wg-quick", "up", "wg0"], log_source="vpn")
    if result == "":
...
def stop_wireguard() -> bool:
    run(["wg-quick", "down", "wg0"], log_source="vpn")
...
def start_openvpn(config: str, name: str = "travel-router") -> bool:
    try:
        ...
        with open(ovpn_conf, "w") as f:
            f.write(config)
        set_permissions(ovpn_conf, 0o600, "vpn")

        result = run(["systemctl", "start", f"openvpn@{name}"], log_source="vpn")
        if result == "":
...
def stop_openvpn(name: str = "travel-router") -> bool:
    run(["systemctl", "stop", f"openvpn@{name}"], log_source="vpn")
...
def get_vpn_status() -> bool:
    result = run(["ip", "link", "show", "wg0"], log_source="vpn")
    return bool(result)
